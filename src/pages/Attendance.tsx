...
const mockAttendanceData = {
  ...
  calendarEvents: {
    '2023-11-15': [
      { courseCode: 'HIST101', status: 'absent' }
    ],
    '2023-11-17': [
      { courseCode: 'ENG102', status: 'absent' }
    ]
  }
};
...
const getPerformanceColor = (percentage: number, threshold = 75) => {
  ...
  if (percentage >= threshold) return 'text-blue-600';
  if (percentage >= threshold - 10) return 'text-amber-600';
  return 'text-red-600';
};

const getProgressColor = (percentage: number, threshold = 75) => {
  if (percentage >= 90) return 'bg-green-500';
  if (percentage >= threshold) return 'bg-blue-500';
